...
int main(int argc, char* argv[]) {
    ...
    std::cout << "please enter number of rows you prefer to play of range [3:16]\n";
    num_rows = getOnlyNumber(0);

    int HumanVsHuman;
    std::cout << "for [human vs machine] enter 0\n";
    std::cout << "for [human vs human] enter any other number\n";
    ...
    while (!ST.game_over(Input, static_cast<size_t>(num_trial)))
        ;
    double t_elaps = ((finish - start) / CLOCKS_PER_SEC);
    cout << "Time elapsed = " <<t_elaps<<endl ;
}